...
public class DamageScenarioService(
    ...
    AccessControlService accessControlService, AssetService assetService)
    : IDataService<DamageScenario>
{
    ...
    public async Task<DamageScenario?> CreateNew(long idAsset)
    {
        ...
        if (asset is null or { ItemDefinition: null }
            or { ItemDefinition.Project: null } ||
            (await accessControlService.CheckUserAccessRightsWrite(
                asset.ItemDefinition.Project.Id) is false))
        {
            return null;
        }

        context.Attach(asset);
        IQueryable<DamageScenario> existingDamageScenarios =
            context.DamageScenarios.Where(e => e.Asset != null &&
                                               e.Asset.ItemDefinition != null &&
                                               e.Asset.ItemDefinition.IdProject ==
                                                   asset.ItemDefinition.IdProject);

        DamageScenario damageScenario =
            new()
            {
                Description = "New unnamed Damage Scenario",
                DamageScenarioNumber =
                        existingDamageScenarios.Any()
                            ? existingDamageScenarios
                                      .Select(e => e.DamageScenarioNumber)
                                      .Max() +
                                  1
                            : 1,
                Asset = asset
    ...
    public async Task Delete(DamageScenario entityToDelete)
    {
            ...
            await accessControlService.CheckUserAccessRightsWrite(projId) is true)
        {
            ...
            await context.Projects.Where(p => p.Id == damageScenario.Asset.ItemDefinition.IdProject).ExecuteUpdateAsync(setters => setters.SetProperty(p => p.DateLastChanged, DateTime.UtcNow));
            await context.SaveChangesAsync();
        }
    }
    ...
    public async Task<DamageScenario?> GetItemByIdAsync(
        long id,
        Func<IQueryable<DamageScenario>, IQueryable<DamageScenario>>? include =
            null,
        CancellationToken cancellationToken = default)
    {
        ...
        DamageScenario? damageScenario =
            await damageScenarios.FirstOrDefaultAsync(a => a.Id == id);
        if (damageScenario is not null
            and { Asset.ItemDefinition.IdProject: long projId } &&
    ...
    public GridItemsProvider<DamageScenario> GetItemsProvider(
        long ProjectId,
        Func<IQueryable<DamageScenario>, IQueryable<DamageScenario>>? include =
            null,
        Func<IQueryable<DamageScenario>, IQueryable<DamageScenario>>? filter =
            null)
    {
        return async request =>
        {
            ...
            IQueryable<DamageScenario> damageScenario =
                context.DamageScenarios.AsNoTracking().Where(
                    a => a.Asset!.ItemDefinition!.IdProject == ProjectId);
    ...
    public async Task<(List<DamageScenario>, int TotalItems)> GetItems(
        long ProjectId,
        Func<IQueryable<DamageScenario>, IQueryable<DamageScenario>>? include =
            null,
        Func<IQueryable<DamageScenario>, IQueryable<DamageScenario>>? filter =
            null)
    {
        ...
        damageScenarios = damageScenarios.Where(
            d => d.Asset!.ItemDefinition!.IdProject == ProjectId);
        if (filter is not null)
        {
            damageScenarios = filter(damageScenarios);
        }

        List<DamageScenario> items = await damageScenarios.ToListAsync();
